...
[
  ...
  teleor-store          ;; Addition for Teleoreactive turtles
]
...
to setup-teleo
  ...
  create-shepherds num-shepherds
    [ set color brown
      ...
      set found-herd? false
      setxy random-xcor random-ycor
      tr-code-of-shepherds ;;; Loading the TR Code
      ]
  reset-ticks
end
...
;  ;;;;
  ...
  ;;; CODE for sheep (Common in both models)
  ask sheep with [not hidden?]
  [ wiggle
    fd sheep-speed ]
  tick
...
to tr-code-of-shepherds
  tr-init
  belief-update-function [[] -> update-beliefs-shepherds]
  beliefs ["sheep-here"  "empty-spot"  "carrying-sheep" "found-herd"]
  durative-actions ["wiggle" "move-away" "move-forward"]
  discrete-actions ["pick-up-sheep" "put-down-sheep" ]
  procedure "default"
    # "carrying-sheep" & "found-herd" & "empty-spot" --> ["put-down-sheep" "move-away"] ++ [[]-> no-belief "found-herd"] .
    # "carrying-sheep" & "sheep-here"  --> ["wiggle" "move-forward"] ++ [[]-> add-belief "found-herd"] .
    # "sheep-here" --> ["pick-up-sheep" "wiggle" "move-forward"] .
    # "true" --> ["wiggle" "move-forward"] .
  end-procedure
  set-goal "default"
end

to update-beliefs-shepherds
  ifelse any? sheep-here with [not hidden?] [add-belief "sheep-here"][no-belief "sheep-here"]
  ifelse all? sheep-here [hidden?] [add-belief "empty-spot"][no-belief "empty-spot"]
  ifelse carried-sheep != nobody [add-belief "carrying-sheep"][no-belief "carrying-sheep"]
end
...
;; One more action needed for moving.
to move-forward
  fd 1
end
...
;;;;;;; Code for running GUI Experiments
...
to experiment
  ...
  [ [x] ->
     ...
     repeat 5 [
      ...
      reset-timer
      while [ticks <= 1000] [go]
      print (word x "  " timer " " herding-efficiency)]
  ]
end
...
reset-timer</setup>
    ...
    <enumeratedValueSet variable="num-sheep">
      <value value="500"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-shepherds">
      <value value="25"/>
      <value value="50"/>
      <value value="75"/>
      <value value="100"/>
  ...
  <experiment name="experiment-teleo" repetitions="3" runMetricsEveryStep="false">
    <setup>random-seed seed
setup-teleo
reset-timer</setup>
    <go>go-teleo</go>
  ...
  <experiment name="experiment-all-on" repetitions="5" runMetricsEveryStep="false">
...
reset-timer</setup>
  ...
  <experiment name="experiment-teleo-all-on" repetitions="5" runMetricsEveryStep="false">
    <setup>random-seed 10
setup-teleo
reset-timer</setup>
    <go>go-teleo</go>
