...
to go-tick  ;; turtle procedure
  ask turtles [
    if color = white [search-for-chip]
    if color = orange [find-new-pile]
    if color = orange and pcolor = black [put-down-a-chip]
    if color = orange and pcolor = black [look-for-space]
    if color = red and pcolor != black [get-away]
  ]
  tick
end
...
to put-down-a-chip  ;; turtle procedure -- finds empty spot & drops chip
  set pcolor yellow
  set color red
end
...
;;;;;;;;;;;;;;;;; Teleoreactive Approach

to tr-code-of-termites
  tr-init
  belief-update-function [[] -> update-termite-beliefs]
  beliefs ["at-chip" "at-pile" "see-pile" "holding-chip" "can-move"]
  durative-actions ["wiggle" "get-away" "do-jump"]
  discrete-actions ["put-down-chip" "pick-up-chip" ]
  procedure "default"

    # "at-chip" --> ["pick-up-chip" "do-jump"] .
    # "true" --> "wiggle" .


  # "picks up chip" & "at-pile" --> "put-down-chip" wait-repeat 2 10  ++ [[]-> show "at-pile - put-down-chip"] .
    # "picks up chip" & "see-pile" --> ["blink" "wiggle"]  .
    # "picks up chip" --> "rotate" .
    # "see-pile" & "holding-chip" --> "picks up chip" .
    # "see-pile" & "wiggle" --> "wiggle" .
    # "see-pile"  --> "rotate" .
    # "true" --> ["blink"  "rotate"  "wiggle"] ++ [ [] -> show "search-for-chip"] .
   end-procedure
  set-goal "default"
end
...
to update-termite-beliefs
 ifelse pcolor = yellow
  [add-belief "at-chip"][no-belief "at-chip"]

; ifelse neighbors pcolor = yellow patches in-radius 3 view-distance view-angle
;   [add-belief "see-pile"]  [no-belief "see-pile"]
; ifelse neighbors pcolor = yellow patches in-radius 0.5
;   [add-belief "at-pile"]  [no-belief "at-pile"]
; ifelse neighbors pile patches in-radius 0.5 view-distance view-angle
;   [add-belief "put-down-chip"] [no-belief "put-down-chip"]
; ifelse neighbors chips patches in-radius 1
;   [add-belief "picks up chip"] [no-belief "picks up chip"]
; ifelse can-move? 0.2 [add-belief "can-move"] [no-belief "can-move"]
end
...
; Updated with Teleoreactive model 2022::: Ilias Sakellariou & Vasileios Apostolidis-Afentoulis.
